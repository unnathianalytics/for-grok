                                ...
                                <tr wire:key="row-{{ $index }}">
                                    ...
                                    <td style="width: 20%">
                                        <div wire:ignore>
                                            <input type="text" class="form-control item-autocomplete"
                                                data-index="{{ $index }}" id="item_name_{{ $index }}"
                                                value="{{ $item['item_name'] ?? '' }}" placeholder="Search item...">
                                        </div>
                                        <input type="text" class="item-id-input" id="item_id_{{ $index }}"
                                            wire:model.debounce.500ms="invoiceItems.{{ $index }}.item_id">
                                    </td>
                                    <td style="width: 5%">
                                        <input type="number" style="text-align:right" step="0.01" placeholder="Qty"
                                            class="form-control focusable" id="quantity_{{ $index }}"
                                    ...
                                    <td>
                                        <input type="text" style="text-align:right" placeholder="GST%"
                                            wire:model.lazy="invoiceItems.{{ $index }}.gst_percent" disabled
                                            readonly>
                                    </td>
                                    <td>
                                        <input type="number" style="text-align:right" step="0.01"
                        ...
                        <tr>
                            <td class="m-0 border-0">
                                <div class="m-0 border-0">
                                    <button type="button" class="btn btn-primary btn-sm" wire:click="addItemRow">Add
                                        Item</button>
                                </div>
                            </td>
                            ...
                            <table class="table table-sm">
                                ...
                                <tbody>
                                    @foreach ($sundries as $index => $sundry)
                                        <tr wire:key="sundry-{{ $index }}">
                                            ...
                                            <td>
                                                <input type="number" style="text-align:right" placeholder="Amount"
                                ...
                                <thead>
                                    <tr>
                                        ...
                                        <th style="width: 10%; text-align: end;font-weight:bold; font-size:1.2rem;">
                                            {{ Number::currency($sundrySubTotal + $itemSubTotal, 'INR') }}
                                        </th>
                                        <th style="width: 5%;"></th>
                                    </tr>
                                </thead>
                                                ...
                                                <td>
                                                    <select class="form-select form-select-sm"
                                                        ...
                                                        @php
                                                            $allowedCategories = [];
                                                            if (
                                                                $invoiceTypeModel->transaction_category === 'return' &&
                                                                in_array($invoiceTypeModel->slug, ['sale-return'])
                                                            ) {
                                                                $allowedCategories = ['sale'];
                                                            } elseif (
                                                                $invoiceTypeModel->transaction_category === 'return' &&
                                                                in_array($invoiceTypeModel->slug, ['purchase-return'])
                                                            ) {
                                                                $allowedCategories = ['purchase'];
                                                            } elseif (
                                                                $invoiceTypeModel->transaction_category === 'material'
                                                            ) {
