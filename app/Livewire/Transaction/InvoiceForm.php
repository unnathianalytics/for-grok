...
use App\Models\{Account, BillSundry, TaxType, Item, Invoice, VoucherSeries, ItemUom, Uom, InvoiceSundry, InvoiceType, SerialNumber, InvoiceItem, InvoiceItemSerialNumber, MaterialCenter, InvoicePrintConfig};
...
class InvoiceForm extends Component
{
    ...
    public $invoice_number, $invoice_date, $voucher_series_id, $mc_id, $tax_type_id, $invoice_time, $account_id, $description;
    ...
    public $defaultItemValues = [
        ...
        'serial_numbers' => [],
        'material_center_id' => null, // Added for MaterialCenter
    ];
    // Print
    public $printFormats;
    ...
    public function mount(string $invoiceType, ?int $invoiceId = null)
    {
        ...
        $this->allMCs = MaterialCenter::all();
        ...
        $this->mc_id = $this->invoice->material_center_id; // Set mc_id for edit mode
        ...
        foreach ($this->invoice->items as $item) {
            ...
            $this->invoiceItems[] = array_merge($this->defaultItemValues, [
                ...
                'material_center_id' => $item->material_center_id, // Added for MaterialCenter
            ]);
    ...
    {
        return Invoice::with(['items.materialCenter', 'invoiceSundries'])->findOrFail($id)->fresh();
    }
    ...
    {
        $this->invoiceItems[] = array_merge($this->defaultItemValues, [
            'material_center_id' => $this->mc_id, // Set material_center_id from mc_id
        ]);
    ...
    public function updatedMcId($value)
    {
        ...
        foreach ($this->invoiceItems as $index => $item) {
            $this->invoiceItems[$index]['material_center_id'] = $value;
        }
    }
    ...
    public function save()
    {
        ...
        $rules = [
            'mc_id' => 'required|exists:material_centers,id',
            ...
            'invoiceItems.*.material_center_id' => 'nullable|exists:material_centers,id',
        ...
        $this->validate($rules, [
            'mc_id.required' => 'Material Center is required.',
            ...
            'invoiceItems.*.sale_price.numeric' => 'Sale price must be a number for row :index.',
            ...
            'invoiceItems.*.serial_numbers.required' => 'Serial numbers are required for item :attribute in row :index.',
            'invoiceItems.*.serial_numbers.size' => 'Exactly :size serial numbers are required for item :attribute in row :index.',
            'invoiceItems.*.serial_numbers.*.serial_number.required' => 'Serial number is required for item :attribute in row :index.',
            'invoiceItems.*.serial_numbers.*.serial_number.max' => 'Serial number must not exceed 55 characters for item :attribute in row :index.',
            'invoiceItems.*.serial_numbers.*.description.max' => 'Description must not exceed 55 characters for item :attribute in row :index.',
            'invoiceItems.*.serial_numbers.*.id.required' => 'Serial number selection is required for item :attribute in row :index.',
        ]);
        ...
        $this->invoice->material_center_id = $this->mc_id; // Save mc_id to Invoice
        $this->invoice->save();
        ...
        foreach ($this->invoiceItems as $index => $itemData) {
            ...
            $invoiceItem = $this->invoice->items()->create([
                ...
                'material_center_id' => $this->mc_id, // Use mc_id from main form
